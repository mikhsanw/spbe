...
const APP_ENC=FALSE;
function callAPI($endpoint, $operation, $accesskey='', $parameter=[], $callmethod='REST', $outputformat='array')
{
    $result=FALSE;
    $agent="MANTRA";
    $rootkeytag='response';
    ...
    if (empty($endpoint)) {
        $response=['status'=>0, 'code'=>20001, 'message'=>'URL/EndPoint tidak terdefinisi (kosong)', 'data'=>''];
        $axml=[$rootkeytag=>$response];
    }
    else {
        ...
        if (in_array($callmethod, ['GET', 'POST']) && !empty($parameter)) {
        ...
        if (empty($uri)) {
            $response=['status'=>0, 'code'=>20002, 'message'=>'URI tidak terdefinisi (kosong)', 'data'=>''];
            $axml=[$rootkeytag=>$response];
        }
        else {
            $uri=substr($uri, -1) == '/' ? substr($uri, 0, -1) : $uri;
            ...
            if (in_array($callmethod, ['GET', 'REST', 'RESTFULL', 'RESTFULLPAR'])) {
            ...
            // Periksa kesalahan
            if ($errno != 0) {
                $response=['status'=>0, 'code'=>$errno, 'message'=>$errmsg, 'data'=>''];
                $axml=[$rootkeytag=>$response];
            }
            else {
                if (APP_ENC) {
                    $content=dec64data($content);
                }
                if (substr($content, 0, 5) == '<?xml' || substr($content, 0, 5) == '<ows:') {
                    $acontent=setXML2Array($content);
                    if (!isset($acontent[$rootkeytag]['status'])) {
                        $response=['status'=>1, 'code'=>200, 'message'=>'OK', 'data'=>$acontent];
                        $axml=[$rootkeytag=>$response];
                    }
                    else {
                        $axml=$acontent;
                ...
                else if (substr($content, 0, 1) == '{' && substr($content, -1) == '}') {
    ...
    if (!empty($axml)) switch ($outputformat) {
        case "xml":
            try {
                $result=setArray2XML($rootkeytag, $axml[$rootkeytag]);
            } catch (exception $e) {
                $response=['status'=>0, 'code'=>20003, 'message'=>$e->getMessage(), 'data'=>''];
                $axml=[$rootkeytag=>$response];
                $result=setArray2XML($rootkeytag, $axml[$rootkeytag]);
            }
            break;
...
function getAPI($endpoint, $operation, $accesskey='', $parameter=[], $callmethod='REST')
{
    return callAPI($endpoint, $operation, $accesskey, $parameter, $callmethod);
}

function getAPIJSON($endpoint, $operation, $accesskey='', $parameter=[], $callmethod='REST')
{
    return callAPI($endpoint, $operation, $accesskey, $parameter, $callmethod, "json");
}

function getAPIXML($endpoint, $operation, $accesskey='', $parameter=[], $callmethod='REST')
{
...
function setArray2XML($nodename, $data)
{
    ...
    return Array2XML::createXML($nodename, $data)->saveXML();
}
        ...
        if (is_array($arr)) {
            ...
            if (isset($arr['@attributes'])) {
                foreach ($arr['@attributes'] as $key=>$value) {
                    ...
                    $node->setAttribute($vkey, self::bool2str($value));
                }
            ...
            foreach ($arr as $key=>$value) {
                ...
                if (is_array($value) && is_numeric(key($value))) {
                    ...
                    foreach ($value as $k=>$v) {
                        $node->appendChild(self::convert($vkey, $v));
                    }
                }
                else {
                    // ONLY ONE NODE OF ITS KIND
                    $node->appendChild(self::convert($vkey, $value));
                }
        ...
        if (is_array($data) and (count($data) > 0)) {
            ...
            for ($page=1; $page <= $maxpage; $page++) {
                ...
                foreach ($header as $key=>$field) {
                    echo "<th style='padding:4px;background-color:#008bca;color:#fff;'>".strtoupper($key)."</th>";
                }
                echo '</tr>';
