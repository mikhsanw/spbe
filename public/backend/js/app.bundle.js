...
var initApp = (function(app) {
	...
	app.listFilter = function (list, input, anchor) {
		...
		$(input).change( function () {
			...
			if(filter.length > 1) {
                /* Change first string to UpperCase */
                filter = filter.replace(/\b[a-z]/g, function (letter) {
    ...
    /** Pengaturan untuk pengumuman **/
    app.topAnnouncement= function () {
        let topInfo = document.getElementById('top-information')
        if(topInfo != null) {
            let kode = topInfo.attributes['kode-halaman'].value;
            if (sessionStorage.getItem('status_pengumuman') === kode) {
                topInfo.style.display = 'none'
            } else {
                topInfo.style.display = 'block'
            }
            document.getElementsByClassName('close-top-information')[0].addEventListener('click', function () {
                sessionStorage.setItem('status_pengumuman', kode ?? 'alert-info');
                topInfo.style.display = 'none'
            });
        }
    }
...
jQuery(document).ready(function() {
    ...
    /**
     * Setting pengumuman atas
     */
    initApp.topAnnouncement();
});
