  ...
  $.loadmodal = function (options) {
    ...
    options.ajax.success.unshift(function (data, status, xhr) { // unshift puts this as the first success method
      ...
      var div = $([
        ...
        '           <button class="btn btn-sm btn-danger tutup-modal" data-dismiss="modal"><i class="fa fa-ban"></i> Tutup</button>',
        '           <button type="button" class="btn btn-info btn-sm kirim-modal float-right btn-' + options.bgClass + ' submit-' + submitClass + '" style="' + options.deeStel + '"><i class="fa fa-refresh fa-spin loading" style="display:none;"></i><i class="fa fa-save"></i> ' + options.title + '</button>',
        '        </div>',
        '      </div>',
        '    </div>',
        '  </div>',
